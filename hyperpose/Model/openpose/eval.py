...
def multiscale_search(img,model):
    ...
    for scale in scales:
        ...
        #print(f"test before transform input_img.shape:{input_img.shape}")
...
def infer_one_img(model,post_processor:PostProcessor,img,img_id=-1,enable_multiscale_search=False,is_visual=False,save_dir="./vis_dir"):
    ...
    humans = post_processor.process(predict_x,data_format=data_format)
    if(is_visual):
        if(data_format=="channels_first"):
