    ...
    resized_images.reserve(image_files.size());
    {
        ...
        for (int i = 0; i < image_files.size(); ++i) {
            ...
            resized_images.push_back(
                cv::Mat(cv::Size(width, height), CV_8UC(3), hwc_images[i].data()));
        }
    }
    {
        TRACE_SCOPE("batch run tensorRT");
        (*compute_feature_maps)({ chw_images.data() }, { pafs.data(), confs.data() },
            image_files.size());
    }
    {
        ...
        auto task = [start_idx, this, &resized_images](std::size_t i,
            ...
            const auto humans = [this, i, use_gpu]() {
                ...
                std::cout << pafs[i].data()[2] << ' ' << confs[i].data()[2]
                          << std::endl;
...
void pose_detector_impl::inference(
