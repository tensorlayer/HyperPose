...
struct default_inputer : stream_detector::inputer_t {
        ...
        float* chw_ptr) override
    {
        ...
        input_image(filenames[idx++],
            ...
            ttl::tensor_ref<float, 3>(chw_ptr, 3, height, width), flip_rgb);
        return true;
    }
};
...
public:
    ...
    void run(inputer_t& in, handler_t& handle, int count) override
    {
        ...
        for (int i = 0; i < buffer_size; ++i) {
            ...
            feature_stream_1.put(feature_stream_t{ confs[i].data(), pafs[i].data() });
        }
        ...
        ths.push_back(std::thread([&]() {
            for (int i = 0; i < count; ++i) {
                ...
                (*compute_feature_maps)({ p.chw_ptr }, { q.paf_ptr, q.heatmap_ptr }, 1);
        ...
        ths.push_back(std::thread([&]() {
            for (int i = 0; i < count; ++i) {
                ...
                const auto humans = (*process_paf)(q.heatmap_ptr, q.paf_ptr, false);
                printf("got %lu humnas from %d-th image\n", humans.size(), i);
                bool draw_humans = true;
                if (draw_humans) {
                    cv::Mat resized_image(cv::Size(width, height), CV_8UC(3), p.hwc_ptr);
