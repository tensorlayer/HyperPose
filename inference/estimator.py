...
class Human:
    ...
    def get_face_box(self, img_w, img_h, mode=0):
        ...
        parts = [
            part for idx, part in self.body_parts.items()
...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(368, 368), tf_config=None):
        ...
        x = tf.placeholder(tf.float32,
                           [None, target_size[1], target_size[0], 3], "image")
        ...
        self.persistent_sess.run(
            tf.variables_initializer([
                ...
                if v.name.split(':')[0] in ['smoothing/gauss_weight']
            ]))
    ...
    def inference(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        logger.debug('inference+ original shape=%dx%d' % (npimg.shape[1],
                                                          npimg.shape[0]))
        img = npimg
        if resize_to_default:
...
if __name__ == '__main__':
    ...
    t = time.time()
    humans = PoseEstimator.estimate_paf(data['peaks'], data['heatMat'],
                                        data['pafMat'])
    dt = time.time() - t
    t = time.time()
