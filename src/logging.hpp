...
template <typename... Args> std::ostream &info(const Args &... args)
{
    get_info_logger() << "[SwiftPost::INFO   ] ";
    ((get_info_logger() << args), ...);
}

template <typename... Args> std::ostream &warning(const Args &... args)
{
    get_warning_logger() << "[SwiftPost::WARNING] ";
    ((get_warning_logger() << args), ...);
}

template <typename... Args> std::ostream &error(const Args &... args)
{
