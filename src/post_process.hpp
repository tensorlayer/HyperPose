...
public:
    peak_finder_t(int channel, int height, int width, int ksize)
            : channel(channel), height(height), width(width), ksize(ksize), smoothed_cpu(channel, height, width),
              pooled_cpu(channel, height, width), same_max_pool_3x3_gpu(1, channel, height, width, 3, 3) {
    }
