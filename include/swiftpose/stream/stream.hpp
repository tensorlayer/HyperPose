...
public:
    ...
    void resize_from_inputs(cv::Size size) {
        ...
        auto&& f = std::async([this, &inputs]{
            m_input_queue_replica.push(inputs);
        });
    ...
    void parse_from_internals(ParserList&& parser_list) {
        ...
        m_cv_dnn_inf.wait(lk, [this]{return m_after_inference_queue.m_size > 0;});
        ...
        for (size_t round_robin = 0; round_robin < internals.size(); ++round_robin) {
            futures.push_back(
                    m_thread_pool.enqueue(
                            [&] (size_t robin){
                                return parser_list.at(robin % parser_list.size()).process(std::move(internals[round_robin]));
                                }, round_robin));
    ...
    void write_to_opencv_video(cv::VideoWriter& writer) {
        ...
        m_cv_post_processing.wait(lk, [this]{return m_pose_sets_queue.m_size > 0;});
    ...
    void write_to_opencv_images(NameGetter&& name_getter) {
        static_assert(
                std::is_same_v<std::result_of_t<NameGetter()>, std::string>,
    ...
    void add_queue_monitor(double milli = 1000) {
        ...
        m_thread_tracer.push_back(std::async([this, milli]{
            while(!m_shutdown) {
                ...
                std::cout << "thread_safe_queue<cv::Mat> m_input_queue -> Size = " << m_input_queue.unsafe_size() << '\n'
                          << "thread_safe_queue<cv::Mat> m_input_queue_replica -> Size = " << m_input_queue_replica.unsafe_size() << '\n'
                          << "thread_safe_queue<cv::Mat> m_resized_queue -> Size = " << m_resized_queue.unsafe_size() << '\n'
                          << "thread_safe_queue<internal_t> m_after_inference_queue -> Size = " << m_after_inference_queue.unsafe_size() << '\n'
                          << "thread_safe_queue<pose_set> m_pose_sets_queue -> Size = " << m_pose_sets_queue.unsafe_size() << std::endl;
...
{
  public:
    stream(DNNEngine& engine, Parser& parser, size_t parser_replica = 4, size_t queue_max_size = 64)
    :
...
template <typename ... TypeParams, typename ... Others>
stream<TypeParams...> make_stream(TypeParams&& ... type_params, Others&& ... others) {
    return stream<TypeParams...>(std::forward<TypeParams>(type_params)..., std::forward<Others>(others)...);
}
